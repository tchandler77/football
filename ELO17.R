...
#raw_data17 <- readRDS(url('https://raw.githubusercontent.com/guga31bb/nflfastR-data/master/data/play_by_play_2017.rds'))
season <- 2017
#raw_data17 <- nflfastR::load_pbp(season)
data17 <- raw_data17 %>%
...
data17 <- unique(data17)


#insert home_result column, win = 1 lose =0
data17$home_result <- 0
data17$away_result <- 0

data17 <- data17 %>% mutate(home_result = ifelse(home_score_differential > 0, 1, home_result))
data17 <- data17 %>% mutate(home_result = ifelse(home_score_differential == 0, 0.5, home_result))
data17 <- data17 %>% mutate(away_result = abs(1-home_result))

#initialize dvoa data
data17$home_dvoa_rank <- 0
data17$away_dvoa_rank <- 0
data17$home_dvoa <- 0
data17$away_dvoa <- 0

data17$dvoa_rank_diff <- 0
data17$dvoa_diff <- 0
...
teams17 <- teams17[c(2:3)]
teams17 <- unique(teams17)
teams17$ELO <- 1500 
#s is the percent weight given to every team's elo from the previous season
s=20
teams17$ELO <- ((s/100)*teams16$ELO)+(1500*(1-(s/100)))


library(readxl)
X2017_Team_DVOA_Ratings_Overall <- read_excel("C:/Users/tommychandler9/Desktop/FOOTBALL/2017 Team DVOA Ratings Overall.xlsx")
# View(X2017_Team_DVOA_Ratings_Overall)

teams17$DVOA_rank <- X2017_Team_DVOA_Ratings_Overall$`Total DVOA Rank`
teams17$total_DVOA <- X2017_Team_DVOA_Ratings_Overall$`Total DVOA`

#epa
teams17$epa <- total_epa$'2017epa'

# initialize score diff
teams17$score_diff <- 0
data17$total_score_diff_home <- 0 
data17$total_score_diff_away <- 0
data17$diff_score_diff <- 0
#ELO CALCULATIONS
#start updating loop
for(i in 1:nrow(data17)){
  print(i)
  
  team_H <- data17$home_team[i]
  team_A <- data17$away_team[i]
  
  data17$total_score_diff_home[i] <- as.numeric(teams17[teams17$team == team_H, "score_diff"])
  data17$total_score_diff_away[i] <- as.numeric(teams17[teams17$team == team_A, "score_diff"])
  diff_score_diff <- as.numeric(data17$total_score_diff_home[i] - data17$total_score_diff_away[i])
  data17$diff_score_diff[i] <- diff_score_diff
  
  score_diff <- data17$home_score_differential[i]
  score_diff_home <- score_diff 
  score_diff_away <- -1 * score_diff 
  
  total_score_diff_home <-   teams17[teams17$team == team_H, "score_diff"] + score_diff_home
  total_score_diff_away <-   teams17[teams17$team == team_A, "score_diff"] + score_diff_away
  
 
  teams17[teams17$team == team_H, "score_diff"] <- total_score_diff_home
  teams17[teams17$team == team_A, "score_diff"] <- total_score_diff_away
  
  
  Result_H <- data17$home_result[i]
  Result_A <- data17$away_result[i]
  
  #get dvoa (these values are end of season totals)
  DVOA_rank_H <- as.numeric(teams17[teams17$team == team_H,"DVOA_rank"])
  DVOA_rank_A <- as.numeric(teams17[teams17$team == team_A,"DVOA_rank"])
  DVOA_H <- as.numeric(teams17[teams17$team == team_H,"total_DVOA"])
  DVOA_A <- as.numeric(teams17[teams17$team == team_A,"total_DVOA"])
  DVOA_rank_diff <- DVOA_rank_H - DVOA_rank_A
  DVOA_diff <- DVOA_H - DVOA_A
  
  #load dvoa into dataset
  data17$home_dvoa_rank[i] <- DVOA_rank_H
  data17$away_dvoa_rank[i] <- DVOA_rank_A
  data17$home_dvoa[i] <- DVOA_H
  data17$away_dvoa[i] <- DVOA_A
  
  data17$dvoa_rank_diff[i] <- DVOA_rank_diff
  data17$dvoa_diff[i] <- DVOA_diff
  
  #load epa
  epa_h <- as.numeric(teams17[teams17$team == team_H,"epa"])
  epa_a <- as.numeric(teams17[teams17$team == team_A,"epa"])
  epa_diff <-epa_h - epa_a
  
  data17$home_epa[i] <- epa_h
  data17$away_epa[i] <- epa_a
  data17$epa_diff[i] <- epa_diff
  
  #get current elo
  ELO_H <- as.numeric(teams17[teams17$team == team_H, "ELO"])
  ELO_A <- as.numeric(teams17[teams17$team == team_A, "ELO"])
  
  #load current elo into dataset
  data17$home_ELO[i] <- ELO_H + 10
  data17$away_ELO[i] <- ELO_A
  
  #UPDATE ELO 
  R_home <- 10^(data17$home_ELO/400)
  R_away <- 10^(data17$away_ELO/400)
  ...
  # MOV <- MOV * (2.2 / denom1)
  home_spread <- data17$home_spread
  pregame_elo_diff <- abs(ELO_H - ELO_A)
  spread_score_diff <- abs(score_diff - home_spread)
  data17$MOV[i] <- 1 + abs((score_diff/28))
  ...
  data17$ELOhome_win_pct[i] <- 1/(1+10^(-1*(elo_diff/400)))
  ...
  teams17[teams17$team == team_H, "ELO"] <- Elo_Updated_home[i]
  teams17[teams17$team == team_A, "ELO"] <- Elo_Updated_away[i]
...
model_pct <- sum(data17$Elo_Forecast_Result)/nrow(data17) 
...
spread_win_pct <- sum(data17$win_spread)/nrow(data17)
...
spread_win_pct1 <- sum(data17$win_spread1)/nrow(data17)
...
regression_win_pct <- sum(data17$regression_predict_win)/nrow(data17)
